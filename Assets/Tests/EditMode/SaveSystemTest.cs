...
public class SaveSystemTest
{
    ...
    [Test]
    public void RoadUnitTest()
    {
        Road original = RoadBuilder.Single(0, stride, 2 * stride, 1);
        Storage storage = new();
        storage.Save(original);
        Road loaded = new();
        storage.Load(loaded);
    ...
    public void RecoverSingleOneLaneRoad()
    {
        ...
        Assert.True(MyNumerics.IsApproxEqual(2 * stride, lane.EndPos));
    ...
    public void RecoverTwoDisconnectedOneLaneRoad()
    {
        ...
        Assert.True(MyNumerics.IsApproxEqual(5 * stride, road1.EndPos));
    ...
    public void RecoverTwoConnectedOneLaneRoad()
    {
        ...
        Assert.AreEqual(new float3(0), road0.StartPos);
        Assert.AreEqual(2 * stride, road0.EndPos);
        Assert.True(MyNumerics.IsApproxEqual(2 * stride, road1.StartPos));
        Assert.True(MyNumerics.IsApproxEqual(4 * stride, road1.EndPos));
    ...
    public void RecoverConnectedLanes_2to1and1()
    {
        ...
        int savedBytes = SaveSystem.SaveGame();
        int loadedBytes = SaveSystem.LoadGame();
        Road restored1 = Game.Roads[saved1.Id];
        Road restored2 = Game.Roads[saved2.Id];
        Road restored3 = Game.Roads[saved3.Id];
    ...
    public void RecoverIntersection()
    {
        ...
        Assert.True(MyNumerics.IsApproxEqual(saved.EndIntersection.Normal, restored.EndIntersection.Normal));
        Assert.True(MyNumerics.IsApproxEqual(saved.EndIntersection.Tangent, restored.EndIntersection.Tangent));
        Assert.True(MyNumerics.IsApproxEqual(saved.EndIntersection.PointOnInSide, restored.EndIntersection.PointOnInSide));
