...
public class GameSave : IPersistable
{
    ...
    public void Save(Writer writer)
    {
        ...
        writer.Write(CarServiced);

        WriteCollection(writer, Nodes);
        WriteCollection(writer, Roads);
        WriteCollection(writer, Intersections);
        WriteCollection(writer, Lanes);
        WriteCollection(writer, Vertices);
        WriteCollection(writer, Paths);
        WriteCollection(writer, Curves);

        static void WriteCollection<T>(Writer writer, Dictionary<uint, T> dict) where T : IPersistable
        {
            writer.Write(dict.Count);
            foreach (IPersistable item in dict.Values)
                item.Save(writer);
        }
    }
